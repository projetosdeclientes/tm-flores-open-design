class HeroCarousel {
  constructor() {
    this.track = document.getElementById('heroTrack');
    if (!this.track) return;
    this.slides = this.track.querySelectorAll('.hero-slide');
    this.dots = document.querySelectorAll('.hero-dot');
    this.current = 0;
    this.autoInterval = null;
    this.intervalDuration = 6000;
    this.isDragging = false;
    this.startX = 0;
    this.init();
  }

  init() {
    this.startAuto();
    this.bindEvents();
  }

  goTo(index) {
    this.slides[this.current].classList.remove('active');
    this.dots[this.current]?.classList.remove('active');
    this.current = (index + this.slides.length) % this.slides.length;
    this.slides[this.current].classList.add('active');
    this.dots[this.current]?.classList.add('active');
    this.animateSlideTexts(this.slides[this.current]);
  }

  animateSlideTexts(slide) {
    const texts = slide.querySelectorAll('.hero-eyebrow, .hero-headline, .hero-cta');
    texts.forEach(el => {
      el.style.animation = 'none';
      void el.offsetHeight;
      el.style.animation = '';
    });
    const img = slide.querySelector('.hero-parallax-layer img');
    if (img) {
      img.style.animation = 'none';
      void img.offsetHeight;
      img.style.animation = '';
    }
  }

  next() { this.goTo(this.current + 1); }
  prev() { this.goTo(this.current - 1); }

  startAuto() {
    this.autoInterval = setInterval(() => this.next(), this.intervalDuration);
  }

  bindEvents() {
    document.querySelector('.hero-arrow-next')?.addEventListener('click', () => {
      this.next(); this.resetAuto();
    });
    document.querySelector('.hero-arrow-prev')?.addEventListener('click', () => {
      this.prev(); this.resetAuto();
    });

    this.dots.forEach((dot, i) => {
      dot.addEventListener('click', () => { this.goTo(i); this.resetAuto(); });
    });

    this.track.addEventListener('pointerdown', e => {
      this.isDragging = true;
      this.startX = e.clientX;
    });
    this.track.addEventListener('pointerup', e => {
      if (!this.isDragging) return;
      const diff = e.clientX - this.startX;
      if (Math.abs(diff) > 50) {
        if (diff < 0) this.next();
        else this.prev();
        this.resetAuto();
      }
      this.isDragging = false;
    });
  }

  resetAuto() {
    clearInterval(this.autoInterval);
    this.startAuto();
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => new HeroCarousel());
} else {
  new HeroCarousel();
}
